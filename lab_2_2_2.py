...
for i in range(exp_newton_iterations):
    if abs(exp_newton_values[i] - exp_newton_root) < 0.001:
        barier_n = i
        break

fig_7 = plt.figure(figsize=(12, 8))
plt.plot(exp_newton_iterations_array, exp_newton_values, 'c-', linewidth=2, label='Значение уточненного метода Ньютона')
plt.xlabel('Итерации')
plt.axvline(barier_exp_n, color='orange', linestyle='--', linewidth=2, label=f'ε < 0.001 на итерации {barier_exp_n}')
plt.axhline(y=exp_newton_root, color='r', linestyle='--', linewidth=2, label=f"Найденный корень = {exp_newton_root:.4f}")
plt.ylabel('Значение x')
plt.title('Приближение уточненным методом Ньютона')
plt.legend()
plt.grid(True, alpha=0.3, which='both')
plt.show()
